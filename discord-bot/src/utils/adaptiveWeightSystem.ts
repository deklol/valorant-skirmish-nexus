...
export interface ExtendedUserRankData {
  current_rank?: string;
  peak_rank?: string;
  manual_rank_override?: string;
  manual_weight_override?: number;
  use_manual_override?: boolean;
  rank_override_reason?: string;
...
export interface EnhancedAdaptiveResult {
  points: number;
  source: string;
  rank: string;
  metadata?: any;
...
/**
 * Get rank points with manual override support
 */
function getRankPointsWithManualOverride(userData: ExtendedUserRankData): EnhancedAdaptiveResult {
  // Check for manual override first
  if (userData.use_manual_override && userData.manual_rank_override) {
    const overridePoints = userData.manual_weight_override || RANK_POINT_MAPPING[userData.manual_rank_override] || 150;
    return {
      points: overridePoints,
      source: 'manual_override',
      rank: userData.manual_rank_override,
      metadata: {
        overrideReason: userData.rank_override_reason
      }
    };
  }

  // Use current rank if available
  if (userData.current_rank && RANK_POINT_MAPPING[userData.current_rank]) {
    return {
      points: RANK_POINT_MAPPING[userData.current_rank],
      source: 'current_rank',
      rank: userData.current_rank
    };
  }

  // Fall back to peak rank
  if (userData.peak_rank && RANK_POINT_MAPPING[userData.peak_rank]) {
    return {
      points: RANK_POINT_MAPPING[userData.peak_rank],
      source: 'peak_rank',
      rank: userData.peak_rank
    };
  }
  ...
  if (manualResult.source === 'manual_override') {
    return {
      ...
      adaptiveCalculation: {
        ...
        calculationReasoning: `Manual override applied: ${manualResult.rank} (${manualResult.points} points)${manualResult.metadata?.overrideReason ? ` - Reason: ${manualResult.metadata.overrideReason}` : ''}`,
