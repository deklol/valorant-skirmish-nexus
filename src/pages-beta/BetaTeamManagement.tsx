...
const NoTeamView = ({ userId, onTeamCreated }: { userId?: string; onTeamCreated: () => void }) => {
  ...
  const handleCreateTeam = async () => {
    ...
    try {
      // Create the team
      // NOTE: Database trigger 'auto_add_captain_as_member_trigger' automatically
      // adds the captain as a member, so we don't need to manually insert into
      // persistent_team_members here
      const { data: team, error: teamError } = await supabase
        ...
        .insert({
          ...
          owner_id: userId,
          status: 'active',
          // Note: invite_code is auto-generated by 'auto_generate_invite_code_trigger'
        })
        .select()
        .single();
