...
-- Add map_pool column to tournaments table to store tournament-specific active maps
ALTER TABLE tournaments ADD COLUMN map_pool jsonb DEFAULT '[]'::jsonb;

-- Update tournaments table to have a comment explaining the map_pool column
COMMENT ON COLUMN tournaments.map_pool IS 'Array of map IDs that were active when this tournament was created. Used for veto sessions to ensure consistent map pools.';

-- Create function to get tournament map pool
CREATE OR REPLACE FUNCTION public.get_tournament_map_pool(p_tournament_id uuid)
RETURNS TABLE(
  id uuid,
  name text,
  display_name text,
...
STABLE
AS $$
  SELECT m.id, m.name, m.display_name, m.thumbnail_url, m.is_active
  FROM maps m
  JOIN tournaments t ON t.id = p_tournament_id
  WHERE m.id = ANY(
    SELECT jsonb_array_elements_text(t.map_pool)::uuid
  )
  ORDER BY m.display_name;
$$;

-- Create function to capture current active map pool for tournament creation
CREATE OR REPLACE FUNCTION public.capture_active_map_pool()
RETURNS jsonb
LANGUAGE sql
STABLE
AS $$
  SELECT jsonb_agg(id ORDER BY display_name)
  FROM maps
  WHERE is_active = TRUE;
$$;

-- Update perform_veto_action to use tournament map pool instead of global active maps
...
BEGIN
    ...
    IF is_bo1 THEN
        ...
        IF bans_made + 1 = total_bans_needed THEN
            ...
            RETURN 'OK';
        ELSE
            -- Move to next team in ban sequence
            IF bans_made + 2 <= array_length(ban_sequence, 1) THEN
                UPDATE map_veto_sessions
                SET current_turn_team_id = ban_sequence[bans_made + 2]
                WHERE id = p_veto_session_id;
            END IF;
        END IF;
...
-- Populate existing tournaments with current active map pool
UPDATE tournaments 
SET map_pool = (SELECT public.capture_active_map_pool())
WHERE map_pool IS NULL OR map_pool = '[]'::jsonb;

-- Fix the two stuck sessions by setting correct turn team
-- Session f51f1a93-0062-41e5-bcb0-90e4e01a4cad has 1 ban (home banned), so away team should be next
UPDATE map_veto_sessions 
SET current_turn_team_id = away_team_id
WHERE id = 'f51f1a93-0062-41e5-bcb0-90e4e01a4cad';

-- Session 6f247532-5f3c-4249-becb-8f28ecf75f62 has 0 bans, so home team should start
UPDATE map_veto_sessions 
SET current_turn_team_id = home_team_id  
WHERE id = '6f247532-5f3c-4249-becb-8f28ecf75f62';
