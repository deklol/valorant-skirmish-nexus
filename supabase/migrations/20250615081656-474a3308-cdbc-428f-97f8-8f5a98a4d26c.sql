...
-- Fix for: column "action" is of type map_veto_action but expression is of type text
...
BEGIN
    ...
    VALUES (
        ...
        p_team_id,
        p_map_id,
        veto_action_type::map_veto_action,  -- <--- Cast as enum!
    ...
    IF total_bans_needed = bans_made AND remaining_maps = 1 THEN
        ...
        SELECT
            ...
            opposite_team,
            id,
            'pick'::map_veto_action,  -- <--- Cast as enum!
            (SELECT COALESCE(MAX(order_number), 0) + 1 FROM map_veto_actions WHERE veto_session_id = p_veto_session_id),
            NULL
