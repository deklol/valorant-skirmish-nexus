-- Fix veto system issues - Drop existing functions first, then recreate

-- Drop existing conflicting functions
DROP FUNCTION IF EXISTS public.is_team_captain(uuid, uuid);
DROP FUNCTION IF EXISTS public.get_user_role(uuid);

-- 1. Update map_veto_actions RLS policies to allow team members (not just captains)
...
-- 2. Add helper function to check if user is on team
...
-- 3. Add function to check if user is team captain
...
-- 4. Add function to get user role safely
CREATE OR REPLACE FUNCTION public.get_user_role(p_user_id uuid)
RETURNS user_role
LANGUAGE sql
STABLE SECURITY DEFINER
AS $$
  SELECT COALESCE(role, 'user'::user_role) FROM users WHERE id = p_user_id;
$$;

-- 5. Fix veto-related RPC functions to be SECURITY DEFINER
...
-- 6. Fix match creation to inherit tournament veto settings
