-- Fix the current stuck veto session and enhance the perform_veto_action function with better logging and validation

-- 1. Fix the immediate stuck session - set correct turn to away team for position 2
UPDATE map_veto_sessions 
SET current_turn_team_id = '72405635-b689-43f6-9ea5-93b5bbe8d5a2' -- away team should be next for ban 2
WHERE id = '762d5585-a07c-4107-9dd4-872209b3f890'
AND current_turn_team_id = '50855923-3e25-4d63-b63c-2f06c2bcf076'; -- only if still stuck on home team

-- 2. Enhanced perform_veto_action function with comprehensive logging and validation
...
BEGIN
    ...
    debug_info := format('Session: %s, Home: %s, Away: %s, Current Turn: %s, Requesting Team: %s', 
                        p_veto_session_id::text, v_home_team_id::text, v_away_team_id::text, 
                        session_rec.current_turn_team_id::text, p_team_id::text);
    ...
    RAISE LOG 'VETO PROGRESS: Position %, Total bans needed %, Maps in pool %', current_ban_position, total_bans_needed, total_maps_in_pool;
    ...
    IF is_bo1 THEN
        ...
        RAISE LOG 'VETO SEQUENCE: Generated ban sequence %, Current position %', ban_sequence, current_ban_position;
        ...
        RAISE LOG 'VETO ACTION INSERTED: Ban % completed by team %, action ID %', current_ban_position, p_team_id, inserted_action_id;
        ...
        ELSE
            -- Move to next team in sequence - CRITICAL FIX AREA
            next_team_id := ban_sequence[current_ban_position + 1];
            
            RAISE LOG 'VETO TURN SWITCH: From position % (team %) to position % (team %)', 
            ...
            IF NOT FOUND THEN
                RAISE LOG 'VETO ERROR: Failed to update current_turn_team_id to %', next_team_id;
                RETURN 'Failed to update turn sequence';
            END IF;
