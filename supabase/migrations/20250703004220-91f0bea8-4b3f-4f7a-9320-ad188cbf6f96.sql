...
CREATE OR REPLACE FUNCTION public.increment_team_wins(team_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
BEGIN
  ...
  RAISE LOG 'Incremented wins for team %', team_id;
END;
$function$;

CREATE OR REPLACE FUNCTION public.increment_team_losses(team_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
BEGIN
  ...
  -- Log the update for debugging
  RAISE LOG 'Incremented losses for team %', team_id;
END;
$function$;

CREATE OR REPLACE FUNCTION public.increment_team_tournament_wins(team_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
BEGIN
  ...
  -- Log the update for debugging
  RAISE LOG 'Incremented tournament wins for team %', team_id;
END;
$function$;

CREATE OR REPLACE FUNCTION public.increment_team_tournaments_played(team_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
BEGIN
  ...
  -- Log the update for debugging
  RAISE LOG 'Incremented tournaments played for team %', team_id;
END;
$function$;

-- Function to update team's average rank points based on current members
CREATE OR REPLACE FUNCTION public.update_team_avg_rank_points(team_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  RAISE LOG 'Updated rank points for team % - avg: %, total: %', team_id, avg_points, total_points;
END;
$function$;
