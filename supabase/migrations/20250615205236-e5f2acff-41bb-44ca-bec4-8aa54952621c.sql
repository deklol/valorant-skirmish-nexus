...
-- 1. Temporarily instrument perform_veto_action with logging for debugging
...
DECLARE
    ...
    is_bo1 BOOLEAN;
BEGIN
    RAISE NOTICE '[VETO_DEBUG] Called by user: %, team: %, map: %', p_user_id, p_team_id, p_map_id;
    ...
    IF match_rec IS NULL THEN
        RAISE NOTICE '[VETO_DEBUG] match_rec not found for id %', session_rec.match_id;
    ...
    RAISE NOTICE '[VETO_DEBUG] v_team1_id % v_team2_id % home_team_id % away_team_id %', v_team1_id, v_team2_id, v_home_team_id, v_away_team_id;
    ...
    IF NOT is_on_team THEN
        RAISE NOTICE '[VETO_DEBUG] user is not on team';
        RETURN 'User is not a member of this team';
    END IF;

    IF session_rec.current_turn_team_id IS DISTINCT FROM p_team_id THEN
        RAISE NOTICE '[VETO_DEBUG] current_turn_team_id: %, p_team_id: %', session_rec.current_turn_team_id, p_team_id;
    ...
    IF map_rec IS NULL THEN
        RAISE NOTICE '[VETO_DEBUG] map_id invalid or inactive';
    ...
    IF EXISTS (
        ...
        RAISE NOTICE '[VETO_DEBUG] map_id already picked or banned';
    ...
    RAISE NOTICE '[VETO_DEBUG] bans_made %, total_bans_needed %, remaining_maps %, is_bo1 %', bans_made, total_bans_needed, remaining_maps, is_bo1;

    -- 4. Determine action type and veto status
    IF is_bo1 THEN
        -- BO1 logic: Alternate bans between home/away, last map auto-pick, home picks side
        IF bans_made < total_bans_needed THEN
            veto_action_type := 'ban';
        ELSE
            RAISE NOTICE '[VETO_DEBUG] All bans complete, waiting for side selection';
        ...
        IF bans_made + 1 = total_bans_needed THEN
            ...
            RAISE NOTICE '[VETO_DEBUG] Final ban. Set turn to home_team_id: %', v_home_team_id;
            -- DO NOT complete session yet; completion waits for side_choice to be entered.
        ELSE
            -- BO1: alternate turn between HOME and AWAY!
            IF p_team_id = v_home_team_id THEN
                opposite_team := v_away_team_id;
            ELSE
                opposite_team := v_home_team_id;
            END IF;

            RAISE NOTICE '[VETO_DEBUG] After ban, turn should go to %', opposite_team;
    ...
    -- Not BO1, legacy BO3/BO5 flow (unchanged)
...
-- 2. Hotfix: manually set current_turn_team_id for session to Team _dek (away) so the ban turn is correct
